...
naturally_ventilated <- df_models %>%
  filter(cooling_type == "naturally ventilated")
lm_nv_result <- lm(neutral_temp ~ t_out_mean, data = naturally_ventilated)
lm_nv_w <- lm(neutral_temp ~ t_out_mean, weights=records, data = naturally_ventilated)


# Filter the data for mix moded buildings
mixed_mode <- df_models %>%
  filter(cooling_type == "mixed mode")
lm_mm_result <- lm(neutral_temp ~ t_out_mean, weights=records, data = mixed_mode)

# Filter the data for air conditioned buildings
air_conditioned <- df_models %>%
  filter(cooling_type == "air conditioned")
lm_ac_result <- lm(neutral_temp ~ t_out_mean, weights=records, data = air_conditioned)
